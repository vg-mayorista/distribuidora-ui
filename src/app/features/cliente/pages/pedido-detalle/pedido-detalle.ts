...
export class PedidoDetalleClienteComponent implements OnInit {
  ...
  onReceiptFileSelected(event: Event): void {
    ...
    if (file.size > 15 * 1024 * 1024) {
      this.error.set('El comprobante debe pesar menos de 15MB.');
      return;
    }
    this.uploadingReceipt.set(true);
    this.error.set(null);
    this.compressAndReadImage(file, (receiptUrl) => {
      this.orderService.uploadReceipt(o.id!, receiptUrl).subscribe({
        ...
        error: (err) => {
          this.uploadingReceipt.set(false);
          const msg = err?.error?.detail || err?.error?.message || 'No se pudo subir el comprobante. Reintentá.';
          this.error.set(msg);
        }
      });
    });
